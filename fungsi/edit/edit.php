...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['jual'])){
		...
		if($hasil['stok'] > $jumlah)
		{
			...
			$totalawal = $jual * $jumlah;
			$totalreal = $totalawal-($totalawal * $diskon/100);
			$total = $totalreal;
			$data1[] = $jumlah;
			$data1[] = $diskon;
			$data1[] = $total;
	...
	?>
		...
		<?php foreach($hasil1 as $hasil){?>
			<tr>
				...
				<td><?php echo $hasil['harga_jual'];?></td>
				<td>
				<a href="fungsi/tambah/tambah.php?jual=jual&id=<?php echo $hasil['id_barang'];?>&id_kasir=<?php echo $_SESSION['user']['id_member'];?>" 
					class="btn btn-success">
					<i class="fa fa-shopping-cart"></i></a></td>
			</tr>
		<?php }?>
